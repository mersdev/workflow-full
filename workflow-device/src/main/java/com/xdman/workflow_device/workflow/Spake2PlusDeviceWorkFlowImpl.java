...
public class Spake2PlusDeviceWorkFlowImpl implements Spake2PlusDeviceWorkFlow {
  ...
  public void startDeviceOwnerPairing() {
	String status = "";
	Workflow.await(() -> message.message().contains("803000")|| !message.message().isEmpty());
	String oldMessage = message.message();
	...
	try {
	  deviceActivity.sendSpake2PlusResponseSuccessfully(message.vin(), response.response().encode());
	...
	Workflow.await(() -> message.message().contains("803200")|| !message.message().equals(oldMessage));
